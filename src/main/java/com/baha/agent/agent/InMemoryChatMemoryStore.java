...
/**
 * Bounded in-memory {@link ChatMemoryStore}, keyed by conversationId.
 * <ul>
 ...
 * </ul>
 * In-memory only — not persisted across restarts. All access is synchronized:
 * the underlying LinkedHashMap is not thread-safe and access-order mutates on
 * read. Default store; the Postgres store (when the {@code postgres} profile is
 * active) is {@code @Primary} and takes precedence.
 */
@Component
public class InMemoryChatMemoryStore implements ChatMemoryStore {
    ...
    public InMemoryChatMemoryStore(int maxMessages) {
        this(maxMessages, 1000);
    }

    public InMemoryChatMemoryStore(int maxMessages, int maxConversations) {
